dat<-read.csv("O:/SSNERR/Science/Projects_Active/Sentinel Sites/synthesis/NAMASTE_MarshSynthesis_Peter/Analysis Files/Namaste_data/slopesAndExpl_bySite_July2024.csv",header=T)
names(dat)
head(dat)
attach(dat)
...
mod.lme <- lme(EIR_slope~Geomorphology+Salinity_category+SLR_last19yrs+Tidal_Range+NERRs_Landscape_resiliency_condition_sum_quantile+SET_minus_SLR_19yrs+proportion_low, random =~1|Reserve, data=dat2, method="REML") #use REML when looking at final model, use ML when comparing
vif_values<-vif(mod.lme)
vif_values
